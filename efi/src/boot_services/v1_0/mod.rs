...
pub trait EfiBootServicesRevision_1_0: EfiEventAndTimer + EfiImage + EfiMemory + EfiMiscellaneous + EfiProtocolHandler + EfiTaskPriority {}
...
impl EfiEventAndTimer for EfiBootServicesRevision_1_0_Raw {
	fn create_event(&self, event_type: EfiEventType, tpl: EfiTaskPriorityLevel, notify: Option<(EfiEventNotifyCallback, VoidPtr)>) -> EfiStatusEnum<EfiEvent> {
		self.event_and_timer.create_event(event_type, tpl, notify)
	}

	fn set_timer(&self, event: EfiEvent, timer_type: EfiTimerDelay, trigger_time: u64) -> EfiStatusEnum {
		self.event_and_timer.set_timer(event, timer_type, trigger_time)
	}
...
impl EfiImage for EfiBootServicesRevision_1_0_Raw {
	fn load_image(&self, boot_policy: bool, parent_image_handle: EfiHandle, device_path: &EfiDevicePathProcotol, source_buffer: Option<&[u8]>) -> EfiStatusEnum<EfiHandle> {
		self.image.load_image(boot_policy, parent_image_handle, device_path, source_buffer)
	}
...
impl EfiMemory for EfiBootServicesRevision_1_0_Raw {
	fn allocate_pages(&self, allocation_type: EfiAllocateType, memory_type: EfiMemoryType, number_of_pages: usize, physical_address: &mut EfiPhysicalAddress) -> EfiStatusEnum {
		self.memory.allocate_pages(allocation_type, memory_type, number_of_pages, physical_address)
	}
...
impl EfiMiscellaneous for EfiBootServicesRevision_1_0_Raw {
	...
	fn set_watchdog_timer(&self, timeout: usize, watchdog_code: u64, watchdog_data: Option<&[u16]>) -> EfiStatusEnum {
		self.miscellaneous.set_watchdog_timer(timeout, watchdog_code, watchdog_data)
	}
}
...
impl EfiProtocolHandler for EfiBootServicesRevision_1_0_Raw {
	fn install_protocol_interface(&self, handle: &mut EfiHandle, protocol_guid: &EfiGuid, interface_type: EfiInterfaceType, interface: Option<&[u8]>) -> EfiStatusEnum {
		self.protocol_handler.install_protocol_interface(handle, protocol_guid, interface_type, interface)
	}
	...
	fn reinstall_protocol_interface(&self, handle: &mut EfiHandle, protocol_guid: &EfiGuid, old_interface: Option<&[u8]>, new_interface: Option<&[u8]>) -> EfiStatusEnum {
		self.protocol_handler.reinstall_protocol_interface(handle, protocol_guid, old_interface, new_interface)
	}

	fn uninstall_protocol_interface(&self, handle: &mut EfiHandle, protocol_guid: &EfiGuid, interface: Option<&[u8]>) -> EfiStatusEnum {
		self.protocol_handler.uninstall_protocol_interface(handle, protocol_guid, interface)
	}
	...
	fn register_protocol_notify(&self, protocol_guid: &EfiGuid, event: EfiEvent) -> EfiStatusEnum<VoidPtr> {
		self.protocol_handler.register_protocol_notify(protocol_guid, event)
	}
	...
	fn locate_handle<'a>(&self, search_type: EfiLocateSearchType, protocol_guid: Option<&EfiGuid>, search_key: Option<VoidPtr>, buffer: &'a mut [EfiHandle]) -> EfiStatusEnum<&'a [EfiHandle], usize> {
		self.protocol_handler.locate_handle(search_type, protocol_guid, search_key, buffer)
	}

	fn locate_device_path(&self, protocol_guid: &EfiGuid, device_path: &mut EfiDevicePathProcotol) -> EfiStatusEnum<EfiHandle> {
		self.protocol_handler.locate_device_path(protocol_guid, device_path)
	}

	fn install_configuration_table(&self, table_guid: &EfiGuid, table_data: VoidPtr) -> EfiStatusEnum {
		self.protocol_handler.install_configuration_table(table_guid, table_data)
	}
}
...
impl EfiTaskPriority for EfiBootServicesRevision_1_0_Raw {
	...
	fn restore_priority_level(&self, old_priority_level: EfiTaskPriorityLevel) {
		self.task_priority.restore_priority_level(old_priority_level)
	}
}
