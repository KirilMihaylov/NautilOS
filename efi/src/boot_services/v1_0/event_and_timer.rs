...
impl EfiEventAndTimerRaw {
	#[inline(always)]
	pub(super) fn create_event(&self, event_type: EfiEventType, tpl: EfiTaskPriorityLevel, notify: Option<(EfiEventNotifyCallback, VoidPtr)>) -> EfiStatusEnum<EfiEvent> {
	...
	#[inline(always)]
	pub(super) fn set_timer(&self, event: EfiEvent, timer_type: EfiTimerDelay, trigger_time: u64) -> EfiStatusEnum {
		(self.set_timer)(
			event,
			timer_type,
			trigger_time
