...
impl EfiMemoryRaw {
	...
	pub(super) fn get_memory_map(&self, memory_map: &mut [u8]) -> EfiStatusEnum<EfiMemoryDescriptors, usize> {
		let (
			mut allocation_size,
			mut memory_map_key,
			mut descriptor_size,
	...
	pub fn iter<'a>(&self) -> impl Iterator<Item=&'a EfiMemoryDescriptor> {
		EfiMemoryDescriptorIter {
			...
			last_descriptor: unsafe {
				self.descriptors_array.add(
					self.memory_map_size - (self.memory_map_size % self.descriptor_size)
