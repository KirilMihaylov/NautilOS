...
impl EfiMiscellaneous {
	pub fn get_next_high_monotonic_count(&self) -> EfiStatusEnum<u32> {
	...
	pub fn reset(&self, reset_type: EfiResetType, reset_code: EfiStatus, data: &[u8]) -> ! {
		(self.reset)(
			reset_type,
			reset_code,
			data.len(),
