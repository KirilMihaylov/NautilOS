...
impl EfiInfiniBandDevicePath {
    ...
    pub fn port_gid(&self) -> u128 {
        unsafe { (self.port_gid.as_ptr() as *const u128).read_unaligned() }
    }

    pub fn id(&self) -> u64 {
        unsafe { (self.id.as_ptr() as *const u64).read_unaligned() }
    }

    pub fn target_port_id(&self) -> u64 {
        unsafe { (self.target_port_id.as_ptr() as *const u64).read_unaligned() }
    }

    pub fn device_id(&self) -> u64 {
        unsafe { (self.device_id.as_ptr() as *const u64).read_unaligned() }
    }
}
