...
impl EfiSerialAttachedScsiDevicePath {
	pub fn sas_address(&self) -> u64 {
		unsafe {
			(
				self.sas_address.as_ptr() as *const u64
			).read_unaligned()
		}
	}
	...
	pub fn device_and_topology_info(&self) -> EfiSerialAttachedScsiDevicePathDeviceAndTopologyInfo {
		unsafe {
			(
				self.sas_address.as_ptr() as *const EfiSerialAttachedScsiDevicePathDeviceAndTopologyInfo
			).read_unaligned()
		}
	}
...
impl EfiSerialAttachedScsiDevicePathDeviceAndTopologyInfo {
	fn additional_info_bytes(&self) -> Result<EfiSerialAttachedScsiDevicePathAdditionalInfoBytes, u16> {
