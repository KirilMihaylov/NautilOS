...
impl EfiSataDevicePath {
    pub fn hba_port_number(&self) -> u16 {
        unsafe { (self.hba_port_number.as_ptr() as *const u16).read_unaligned() }
    }

    pub fn port_multiplier_port_number(&self) -> u16 {
        unsafe { (self.port_multiplier_port_number.as_ptr() as *const u16).read_unaligned() }
    }

    pub fn logical_unit_number(&self) -> u16 {
        unsafe { (self.logical_unit_number.as_ptr() as *const u16).read_unaligned() }
    }
}

impl EfiDevicePathRepr for EfiSataDevicePath {}
