...
impl EfiiScsiDevicePath {
    pub fn protocol(&self) -> EfiiScsiDevicePathProtocol {
        ...
        let protocol: u16 = unsafe { (self.protocol.as_ptr() as *const u16).read_unaligned() };
    ...
    pub fn options(&self) -> EfiiScsiDevicePathOptions {
        unsafe { (self.options.as_ptr() as *const EfiiScsiDevicePathOptions).read_unaligned() }
    }
    ...
    pub fn target_portal_group_tag(&self) -> u16 {
        unsafe { (self.target_portal_group_tag.as_ptr() as *const u16).read_unaligned() }
    }

    pub fn target_name(&self) -> &[u8] {
        let offset: usize =
            (&self.target_name as *const () as usize) - (self as *const Self as usize);
        let length: usize;
        ...
        unsafe { from_raw_parts(&self.target_name as *const () as *const u8, length) }
    }
}
