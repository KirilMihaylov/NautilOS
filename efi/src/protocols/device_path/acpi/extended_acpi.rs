...
impl EfiExtendedAcpiDevicePath {
    ...
    unsafe fn _uidstr(&self) -> *const u8 {
        ...
        _hidstr_ptr.offset(Self::str_len(_hidstr_ptr) as isize + 1)
    }

    unsafe fn _cidstr(&self) -> *const u8 {
        let _uidstr_ptr: *const u8 = self._uidstr();

        _uidstr_ptr.offset(Self::str_len(_uidstr_ptr) as isize + 1)
    }

    pub fn _hid(&self) -> &[u8] {
        if self._hidstr[0] != 0 {
            let _hid_ptr: *const u8 = self._hidstr.as_ptr();
            unsafe { from_raw_parts(_hid_ptr, Self::str_len(_hid_ptr)) }
        } else {
            &self._hid
        }
    }
    ...
    pub fn _uid(&self) -> &[u8] {
        unsafe {
            ...
            if *_uidstr_ptr == 0 {
                ...
                from_raw_parts(_uidstr_ptr, Self::str_len(_uidstr_ptr))
            }
        }
    }
    ...
    pub fn _cid(&self) -> &[u8] {
        unsafe {
            ...
            if *_cidstr_ptr == 0 {
                ...
                from_raw_parts(_cidstr_ptr, Self::str_len(_cidstr_ptr))
            }
        }
    }
}
