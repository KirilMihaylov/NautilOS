...
use crate::{protocols::network::common::structs::EfiIPAddressRaw, protocols::EfiProtocol, *};
...
impl<'a> From<&'a EfiDevicePathProcotol> for EfiHardwareDevicePathSubtype<'a> {
    fn from(path: &'a EfiDevicePathProcotol) -> Self {
        ...
        match path.path_subtype {
            ...
            6 => match path.len() {
                13 => BaseboardManagementController(
                    EfiBaseboardManagementControllerDevicePath::new(path),
                ),
                _ => Undefined,
            },
            _ => Undefined,
...
impl<'a> From<&'a EfiDevicePathProcotol> for EfiMessagingDevicePathSubtype<'a> {
    fn from(path: &'a EfiDevicePathProcotol) -> Self {
        ...
        match path.path_subtype {
            ...
            10 => match path.len() {
                x if x >= 20 => {
                    let guid: EfiGuid = EfiGuid::from_array(unsafe {
                    ...
                    match guid.as_tuple() {
                        ...
                        ) => UARTFlowControl(EfiUartFlowControlDevicePath::new(path)),
                        (
                            0xd487ddb4,
                            0x008b,
                            0x11d9,
                            [0xaf, 0xdc, 0x00, 0x10, 0x83, 0xff, 0xca, 0x4d],
                        ) => SerialAttachedSCSI(EfiSerialAttachedScsiDevicePath::new(path)),
            ...
            24 => match path.len() {
                x if x >= 4 => {
                    UniversalResourceIdentifier(EfiUniversalResourceIdentifierDevicePath::new(path))
                }
                _ => Undefined,
            },
            25 => match path.len() {
            ...
            31 => match path.len() {
                x if (x as usize - 5) % size_of::<EfiIPAddressRaw>() == 0 => {
                    DomainNameService(EfiDomainNameServiceDevicePath::new(path))
                }
                _ => Undefined,
            },
            32 => match path.len() {
...
impl<'a> From<&'a EfiDevicePathProcotol> for EfiMediaDevicePathSubtype<'a> {
    fn from(path: &'a EfiDevicePathProcotol) -> Self {
        ...
        match path.path_subtype {
            ...
            8 if path.len() == 24 => {
                RelativeOffsetRange(EfiRelativeOffsetRangeDevicePath::new(path))
            }
...
impl EfiDevicePathProcotol {
    ...
    pub fn iter(&self) -> EfiDevicePathProcotolIter<'_> {
        EfiDevicePathProcotolIter { current: self }
    }
    ...
    pub(crate) fn len(&self) -> u16 {
        unsafe { (self.length.as_ptr() as *const u16).read_unaligned() }
    }
...
impl EfiProtocol for EfiDevicePathProcotol {
    fn guid() -> EfiGuid {
        crate::guids::EFI_DEVICE_PATH_PROTOCOL
    }
}
        ...
        if self.current.is_end_of_device_path() {
            ...
            self.current = unsafe {
                &*((self.current as *const EfiDevicePathProcotol as *const u8)
                    .add(self.current.len() as usize)
                    as *const EfiDevicePathProcotol)
            };
            Some(return_item)
        }
    }
...
pub(crate) trait EfiDevicePathRepr: Sized {
    fn new(path: &EfiDevicePathProcotol) -> &Self {
        unsafe { &*(path as *const EfiDevicePathProcotol as *const Self) }
    }
}
