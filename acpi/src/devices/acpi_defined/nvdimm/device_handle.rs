...
use efi_interops::{
    traits::{acpi::device::NvdimmDeviceHandle, conversion::FromEfiObject},
    EfiObject,
};
...
unsafe impl FromEfiObject for NVDIMMDeviceHandle {
    fn convert(object: EfiObject) -> Option<Self> {
        ...
        if object.len() != size_of::<Self>() {
            ...
            let mut new_self: Self = Self { handle: 0 };

            object.copy_over(unsafe {
                from_raw_parts_mut(&mut new_self as *mut Self as *mut u8, size_of::<Self>())
            });
